...
3. Check MCAPS compliance before any Azure architecture decision
4. Use Microsoft Learn MCP or Context7 to verify syntax before running commands
5. Search Obsidian for prior decisions/issues before suggesting solutions
...
- **Challenge is AUTOMATIC** — on any design, build, or approach proposal (Dan's OR your own), do the critical pass unprompted: name alternatives, the top risks, and an explicit "here is where this could be wrong" — BEFORE agreeing or building. Dan should never have to say "red-team this" or "give me alternatives" to trigger this. It is always on.
...
- Track decisions in Obsidian for future reference.
...
- Context files and Obsidian are the source of truth — re-query when uncertain.
- If you notice you're repeating a fix or suggestion, check Obsidian issues/ for prior attempts.
...
1. Search Obsidian issues/ for this error/symptom
...
- **Log tangents, do not chase them.** A bug or idea found mid-task is recorded (in the tangent log / Obsidian issues) and surfaced — not switched to. One task at a time.
...
- **Done-condition tiers** — pick the right tier for this task:
  ...
  - Tier C: human judgment (taste, aesthetics, requirements only Dan can confirm). Say explicitly: "this needs your eyes."
  - Never claim done on a Tier C task without Dan's confirmation. Never fake a Tier A command that trivially exits 0.
...
When Dan asks a decision, choice, approach, or should-we question — without being asked to follow a format:
...
Delivery rules:
- One decision at a time interactively. A backlog becomes one numbered board (D-1, D-2...) presented all at once — Dan rules it in one message.
- Visual choices (layout, color, UI) get a rendered HTML mockup opened BEFORE asking. He decides from pictures, not prose.
- Check the decision record first (Obsidian decisions/ or Logs/decisions/) — never re-ask a question that was already decided.

## 10. HUMAN-IN-THE-LOOP HARD STOP
When Dan is mid an external/manual step that the AI cannot do for him — signing in to a portal, authorizing OAuth, clicking a confirm button in a browser, restarting an app, verifying an email — **STOP and wait for his explicit "done" before proceeding.** Do NOT race ahead and assume the step landed.
This is a HARD STOP, not a preference. Racing ahead when Dan is completing a manual step is a primary cause of silent failure: auth flows that never completed, configs that were never saved, resources that were never created. If in doubt whether Dan has completed the step — ask, don't assume.
