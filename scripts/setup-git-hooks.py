...
def merge_hooks(security_hook, testing_hook, output_hook):
    ...
    testing_content = '\n'.join([line for line in testing_content.split('\n') 
...
def install_hooks():
    ...
    # 1. Merge and install pre-commit hook (security + testing)
    print(f"{YELLOW}📝 Installing pre-commit hook (security + testing)...{NC}")
