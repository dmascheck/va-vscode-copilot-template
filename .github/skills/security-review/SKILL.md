...
### Step 5 — Cross-File Data Flow Analysis + Attacker's-Eye Pass
...
**Attacker's-eye pass (think in chains, not isolated findings):**
- **Map the attack surface first:** enumerate every entry point (HTTP routes, APIs, MCP tools, file/upload handlers, webhooks) and mark what is reachable UNAUTHENTICATED or publicly. The unauthenticated surface is where an attacker starts.
- **Reason about attack CHAINS:** how do two or three individually-minor issues combine into a real breach? (e.g. a verbose error leaks a path → path traversal reads a config → config has an over-scoped token.) A single finding may be Low; the chain may be Critical. Report the chain and rate it by the end impact.
