...
2. **Read the done-condition.** Parse `done_cmd`, `status`, `mode`, and the Objective.
   ...
   - If `done_cmd` is empty (Tier B/C) → there is no machine check. Re-state the Objective and the acceptance criteria, and ask the architect to confirm rather than declaring done yourself.
...
5. **On a clean pass, optionally close the contract.** If the architect agrees, set `status: done` so the next `#start-session` does not resurface it as unfinished.

## Rules
- Report the ACTUAL exit code and real output — running it is the whole point; a claimed pass is not a pass.
- Do NOT edit `done_cmd` to make it pass. If the check itself is wrong, raise that with the architect; do not weaken the gate.
