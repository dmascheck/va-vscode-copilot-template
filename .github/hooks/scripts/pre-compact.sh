...
echo '{
  "systemMessage": "[WARN] COMPACTION IMMINENT — Marathon session checkpoint needed. Before compaction summarizes earlier context: 1) Save a checkpoint summary to Obsidian (key decisions, work completed, current state, next steps). 2) Update PROJECT_CONTEXT.md with current state. 3) Update TODO.md with current task status. This ensures nothing is lost when earlier conversation history is compacted.",
  "continue": true
}'
