...
tools:
  [ 'edit/editFiles', 'web/fetch', 'vscode/runCommand', 'read/terminalLastCommand', 'todos' ]
---
...
- Use `#vscode/runCommand` to verify or create the folder (e.g., `mkdir -p <outputBasePath>`), then proceed.

## Testing & validation

- Use tool `#vscode/runCommand` to run the command for restoring modules: `bicep restore` (required for AVM br/public:\*).
- Use tool `#vscode/runCommand` to run the command for bicep build (--stdout is required): `bicep build {path to bicep file}.bicep --stdout --no-restore`
- Use tool `#vscode/runCommand` to run the command to format the template: `bicep format {path to bicep file}.bicep`
- Use tool `#vscode/runCommand` to run the command to lint the template: `bicep lint {path to bicep file}.bicep`
