...
PATTERNS=(
  ...
  # Azure connection strings and keys (Storage, Service Bus, Event Hub, Relay)
  "AZURE_STORAGE_ACCOUNT_KEY|critical|AccountKey=[A-Za-z0-9+/=]{40,}"
  "AZURE_SHARED_ACCESS_KEY|critical|SharedAccessKey=[A-Za-z0-9+/=]{20,}"
  "AZURE_SB_EVENTHUB_ENDPOINT|high|Endpoint=sb://[^[:space:]'\"]{10,}"
...
scan_file() {
  ...
  for entry in "${PATTERNS[@]}"; do
    ...
    while IFS=: read -r line_num matched_line; do
      ...
      # Skip if this looks like a placeholder or example. Anchor on a delimited
      # token and never excuse a long high-entropy value (real keys are long and
      # may incidentally contain a substring like "fake"/"sample").
      if [[ ${#match} -le 40 ]] && printf '%s\n' "$match" | grep -qiE '(^|[^A-Za-z0-9])(example|placeholder|your[_-]|xxx|changeme|TODO|FIXME|replace[_-]?me|dummy|fake|test[_-]?key|sample)([^A-Za-z0-9]|$)'; then
        continue
      fi
...
if [[ $FINDING_COUNT -gt 0 ]]; then
  ...
  if [[ "$MODE" == "block" ]]; then
    ...
    echo "[TIP] Review the findings above. Set SCAN_MODE=block to prevent commits with secrets."
  fi
else
